...
3. **実装**（ワークツリー内ではバージョンフィールドを変更しない。理由は下記「バージョン管理」参照）
   - 実装中・実装後に `code-review` スキル のチェックリストを突き合わせる。**過去に実際の不具合を出した箇所**（スレッド待ち合わせの取りこぼし・GPU/CPU 二経路の非対称・ネイティブリソースの解放順序・キー入力経路の分散）をまとめてある
...
8. **`ready/*` へ改名**（**改名前に必ずユーザーに確認を取る**。改名は「この内容でリリースしてよい」という宣言にあたる）
   - 確認は **通常の対話** で次の 4 択を提示する
     ...
     | 4 | まだ改名しない | `worktree/*` のまま留める（追加で直す予定がある場合） |

   - 発言がどの選択肢か一意に指している場合（「リリース待ちに入れて」＝1 等）は 4 択の提示を省略してよい。手順 7 でコンフリクト解消を伴った場合は省略せず差分を提示してから確認する
...
> **実行確認はユーザーの環境で行う。Codex 自身は `dotnet run` でアプリを起動しない。**
...
- **実行確認の連携（必須）**: ビルド後、以下をユーザーに提示する。
  - **実行 exe の path**: 必ず絶対パスで連携する（相対パスだと、どのワークツリー／チェックアウトの exe か伝わらず、古い exe を誤って実行される事故につながる）。コピーしやすいよう独立したコードブロックで示す。
    - パスは固定部分と、下表の**可変部分 3 つ**を繋いだ形になる。**組み合わせを例として書き起こさない**（下記の理由）。
      ...
      | 部分 | 決まり方 |
      |---|---|
      | チェックアウト | ワークツリー内でビルドしたなら `<リポジトリ>\.codex\worktrees\<name>`、main 直下なら `<リポジトリ>`。リリースの統合後検証は `release` スキルが main 上での実行を要求するため必ず後者 |
...
| `dotnet run --project src/MultiTrackPlayer.UI` | アプリ起動（**ユーザーが実行確認する際に使うコマンド**。Codex はこれを実行しない） |
