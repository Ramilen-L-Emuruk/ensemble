...
public sealed class TickGate
{
    ...
    /// <remarks>
    /// 戻すのは<b>「いま続いている詰まり」に紐づくものだけ</b>——連続の数えと、その報告の抑制。
    /// <see cref="TickSkipReport.Intermittent"/> 側（累計の数えと、その報告の抑制）は戻さない。
    ...
    /// </para>
    /// <para>
    /// <b>残る競合窓</b>: <c>Sustained</c> の CAS に勝ったスレッドが戻る前に、別のスレッドが
    /// <c>_sustainedReported</c> を 0 と読んで <c>Intermittent</c> を返す窓がごく狭く残る。
    /// ロックを持たない数え方の限界で、代償は「同じ詰まりについて記録が 2 行出る」だけ。
    /// この 2 行に矛盾は無い（文面から「連続では届いていない」という主張を外してある）。
    /// </para>
    /// <para>
