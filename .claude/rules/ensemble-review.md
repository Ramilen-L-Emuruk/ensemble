...
汎用的な C#/.NET のチェックリストは [CLAUDE.md](../../CLAUDE.md)「C# コードレビュー基準」を参照。本ファイルは**このプロジェクトで実際に事故が起きた箇所**だけをまとめる。各項目には根拠となる修正コミット（または、まだコミットが無いものは裏取りしたテスト）を添えてあるので、疑わしいときは `git show <hash>` で当時の変更を確認すること。
...
- [ ] **値を 1 つに統一したら、それを更新する条件も統一すること。** 判定側だけを共有して更新側を
  片方の消費者の都合で書くと、もう片方が待っている隙間で書き換わる。実例: シーク後の猶予の締切
  （`PrerollGraceMs` / `IsWithinSeekGrace`）を映像の滞留検出とクロックの滞留検出で共有したのに、
  置き直す条件が `PrerollGate.IsWaitingForPreroll` だけを見ていた（`a80d1b2`）。**2 つの待ちは同時に解けない**
  （錨の確定＝`PlaybackClock.AnchorAt` は次のミキサー Pull を待つ）ため、片方が待っている間に
  利用者が再生を押すと猶予が延び続け、拾いたい障害が黙る
- [ ] **除外条件が症状そのものを指していないか。** 「正常な待ちだから除外する」という条件が、
  その待ちが解けない不具合そのものを表している場合、無条件に除外すると**その不具合だけが
  永久に検出されない**。時間で縛る（猶予）こと。**設計時に 2 度見つけている**——プリロール待ち
  （`dcf9664` の映像側。`PrerollGate.IsWaitingForPreroll`）とシーク着地待ち
  （`a80d1b2`。`PlaybackClock.IsSeekPending`）。**どちらもレビュー指摘で、本番で踏んだ事故ではない**
  （この節の他の項目と性質が違うので区別して書く）
