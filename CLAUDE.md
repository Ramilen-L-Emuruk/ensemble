...
> 7〜8 と 9 を分けている理由: 複数セッションが並行して作業すると `main` が奪い合いになる。`main` を書き換える操作をリリース手続きに集約し、その主体を常に 1 つに限ることで、実装セッション間の順番調整が不要になる。
...
- 実装フローの手順 7 で行う `git merge main` は逆方向（先行分の取り込み）であって、これとは別物
...
**実行条件（ユーザーから明示的に指示があったときのみ）は上記「変更時の基本フロー」手順 9 を参照。**
