...
- **実行確認の連携（必須）**: ビルド後、以下をユーザーに提示する。
  - **実行 exe の path**:
    - Debug ビルド: `src/MultiTrackPlayer.UI/bin/Debug/net6.0-windows/MultiTrackPlayer.UI.exe`
    - Release ビルド: `src/MultiTrackPlayer.UI/bin/Release/net6.0-windows/MultiTrackPlayer.UI.exe`
...
**ユーザーから明示的に指示があったときのみ**行う。main へのマージ後、main 上で実施する（ワークツリー内では行わない）。
...
- 発行物は `publish/MultiTrackPlayer.UI.exe` と依存 DLL 一式がフラットに並ぶ構成。フォルダごと配布すれば動作する。
...
| `dotnet publish src/MultiTrackPlayer.UI/MultiTrackPlayer.UI.csproj -c Release -o publish` | リリース用 exe 発行（`publish/` 直下にフラット出力） |
