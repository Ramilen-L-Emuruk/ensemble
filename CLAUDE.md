...
8. **バージョン更新**（**main へのマージ直後・main 上で実施**。下記「バージョン管理」参照。マージしない限りこのステップは発生しない）
...
- **実行確認の連携（必須）**: ビルド後、以下をユーザーに提示する。
  - **実行 exe の path**: 必ず絶対パスで連携する（相対パスだと、どのワークツリー／チェックアウトの exe か伝わらず、古い exe を誤って実行される事故につながる）。コピーしやすいよう独立したコードブロックで示す。
    ...
    - Release ビルド例:
      ...
      ```
    - **TFM（パス中の `net10.0-windows` の部分）の単一の情報源は `src/MultiTrackPlayer.UI/MultiTrackPlayer.UI.csproj` の `<TargetFramework>`。** 上の例と食い違っていたら csproj 側が正しい。TFM 移行のたびにこの記述は腐るため、パスを組み立てる前に実際の出力先を確認する（古い TFM のパスを提示すると、存在しない exe や別バージョンの exe をユーザーに実行させる事故になる）。
...
- **手順**（main へのマージ後、毎回省略せず行う）:
  ...
  2. 上げる場合、main 上で `.csproj` の `<Version>` を更新する
  3. バージョン表示箇所が複数ある場合は、可能な限り**単一の情報源**（`.csproj` の `<Version>`）から参照する形にし、手動での二重更新を避ける
...
**GitHub Actions（`.github/workflows/release.yml`）で自動化されている。** 以下のいずれかのトリガーで実行される。

- **タグ push**（推奨）: `v` から始まるタグ（例 `v1.0.0`）を push すると自動実行される
  ```bash
  git tag -a v1.0.0 -m "v1.0.0"
  git push origin v1.0.0
  ```
- **手動実行**: GitHub の Actions タブから「Release」ワークフローを選び、バージョン番号を入力して `Run workflow`

**ユーザーから明示的に指示があったときのみ**行う。main へのマージ・push が完了した状態（＝タグがそのコミットを指す状態）で実施する。
...
| `git tag -a vX.Y.Z -m "vX.Y.Z" && git push origin vX.Y.Z` | GitHub Actions 経由で exe を自動ビルド・Release 公開 |
