...
1. **ワークツリー作成**（修正・機能追加時は必ず `EnterWorktree` でワークツリーに入ってから作業する。ブランチ名は必ず `worktree/<type>/<name>` 形式にする: `worktree/fix/〇〇`・`worktree/feat/〇〇`・`worktree/refactor/〇〇`・`worktree/docs/〇〇`・`worktree/chore/〇〇` など）
2. **実装**
